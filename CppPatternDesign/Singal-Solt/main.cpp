...
class Slot : public SlotBase<T1> {
 public:
  ...
  /* signal触发时，调用 */
  void Exec(T1 param1) { (m_pSlotBase->*m_Func)(param1); }
...
class Signal {
 ...
 private:
  vector<SlotBase<T1>*>
      m_pSlotSet;  //这一句很重要，靠基类的指针来存储 信号槽指针
};

class TestFunc1 {
 public:
  void FuncOfA(int parm) { printf("enter FuncOfA parm = %d\n", parm); }
};
class TestFunc2 {
 public:
  void FuncOfB(int parm) { printf("enter FuncOfB parm = %d\n", parm); }
};

class TestSignal {
 public:
  TestSignal() {}
  void emit(int value) { ValueChanged(value); }
